...
MAX_ITERATION = int(1e5*0.5 + 1)     #最大迭代次数
...
def main(argv=None):
    ...
    with tf.variable_scope('ResNet50',reuse=True):
       logits_label_classify_ul   = model1.ResNet(image_ul, keep_prob,training_flag)
    
    p_label = tf.nn.softmax(logits_label_classify_ul)
    ent_loss =  -tf.reduce_mean(tf.reduce_sum(p_label * logsoftmax(logits_label_classify_ul), 1))

    #loss = ce_loss +  vat_cross_entropy + ent_loss
